// src/pages/dashboard/college/OfferLetter.js
...
const CollegeOfferLetter = () => {
    ...
    useEffect(() => {
        if (appIdFromUrl && candidates.some(c => c._id === appIdFromUrl)) {
    ...
    const eligibleCandidates = useMemo(() => candidates.filter(c => c.user?.employerProfile && ['interview_scheduled', 'documents_verified'].includes(c.status)), [candidates]);
    ...
    const stats = useMemo(() => ({
        ...
        hired: offerProcessCandidates.filter(c => c.status === 'hired').length,
          ...
          <Card>
            ...
            <CardContent><div className="space-y-4">
                {offerProcessCandidates.length > 0 ? offerProcessCandidates.map((app) => {
                    ...
                    return (
                    <div key={app._id} className="p-4 border rounded-lg flex flex-col gap-4">
                        <div className="flex items-center justify-between flex-wrap gap-4">
                            ...
                            <div className="flex items-center gap-3">
                                <Badge className={`${status.color} font-medium`}>{status.label}</Badge>
        ...
        <div className="lg:col-span-1">
          <Card>
              ...
              <CardContent><div className="space-y-3">
                  <div className="flex justify-between items-center"><span className="text-sm text-muted-foreground">In Offer Process</span><span className="font-semibold text-foreground">{stats.total}</span></div>
                  <div className="flex justify-between items-center"><span className="text-sm text-muted-foreground">Pending Admin Approval</span><span className="font-semibold text-yellow-600">{stats.pending}</span></div>
                  <div className="flex justify-between items-center"><span className="text-sm text-muted-foreground">Sent to Candidate</span><span className="font-semibold text-blue-600">{stats.sent}</span></div>
                  <div className="flex justify-between items-center"><span className="text-sm text-muted-foreground">Hired</span><span className="font-semibold text-green-600">{stats.hired}</span></div>
              </div></CardContent>
          </Card>
        </div>
