...
} from "@/features/api/collegeJobApiService";
import {
  Card,
  CardContent,
  CardDescription,
...
const CollegePosts = () => {
  ...
  const stats = useMemo(() => {
    return {
      ...
      closed: jobPosts.filter(
        (p) => p.status === "closed" || p.status === "rejected"
      ).length,
      totalApplicants: jobPosts.reduce(
        (sum, post) => sum + (post.applicants || 0),
        0
      ),
  ...
  const handleEditClick = (jobId: string) =>
    navigate(`/dashboard/college/post-job/edit/${jobId}`);
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        ...
        <Button onClick={() => navigate("/dashboard/college/post-job")}>
      ...
      >
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              This will permanently delete the job post and all associated
              applications.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="p-4 border rounded-lg">
                  ...
                  <div className="text-sm space-y-1 text-muted-foreground">
                    <div>
                      <strong>Department:</strong> {viewingPost.department}
                    </div>
                    <div>
                      <strong>Location:</strong> {viewingPost.location}
                    </div>
                    <div>
                      <strong>Job Type:</strong> {viewingPost.type}
                    </div>
                    <div>
                      <strong>Experience:</strong> {viewingPost.experienceLevel}
                    </div>
                  </div>
                </div>
                <div className="p-4 border rounded-lg">
                  ...
                  <div className="text-sm space-y-1 text-muted-foreground">
                    <div>
                      <strong>Salary:</strong> {viewingPost.salary}
                    </div>
                    {viewingPost.benefits && (
                      <>
                        <strong>Benefits:</strong>
                        <p>{viewingPost.benefits}</p>
                      </>
                    )}
                  </div>
                </div>
