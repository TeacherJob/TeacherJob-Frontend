// src/pages/dashboard/college/Shortlist.js
      ...
      <DialogContent className="max-w-4xl h-[90vh] flex flex-col">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 py-4 overflow-hidden flex-grow">
          <div className="md:col-span-1 space-y-4 overflow-y-auto pr-2 custom-scrollbar border-r">
            <div className="flex flex-col items-center"><div className="w-24 h-24 bg-muted rounded-full flex items-center justify-center mb-4"><User className="w-12 h-12 text-muted-foreground" /></div></div>
            <Separator />
            ...
            <div className="space-y-2 text-sm">
              ...
              <div className="flex items-center gap-2"><Phone size={14} className="text-muted-foreground" /><span>{profile.phone ? maskPhone(profile.phone) : "N/A"}</span></div>
            </div>
            <Separator />
            <h4 className="font-semibold">Skills</h4>
            <div className="flex flex-wrap gap-2">{profile.skills?.length > 0 ? (profile.skills.map((skill: any) => (<Badge key={skill._id} variant="secondary">{skill.name}</Badge>))) : (<p className="text-sm text-muted-foreground">No skills listed.</p>)}</div>
          </div>
...
const CollegeShortlist = () => {
  ...
  const getInterviewStatus = (app: any) => { if (app.status === "hired") return { label: "Hired", color: "bg-green-100 text-green-800" }; if (app.status === "offer_extended") return { label: "Offer Stage", color: "bg-indigo-100 text-indigo-800" }; if (app.status === "interview_scheduled" && app.interviewDetails?.confirmedByAdmin) return { label: "Interview Scheduled", color: "bg-blue-100 text-blue-800" }; if (app.status === "interview_scheduled" && !app.interviewDetails?.confirmedByAdmin) return { label: "Pending Admin", color: "bg-yellow-100 text-yellow-800" }; if (app.status === "rejected") return { label: "Rejected", color: "bg-red-100 text-red-800" }; return { label: "Shortlisted", color: "bg-green-100 text-green-800" }; };
  const filteredCandidates = useMemo(() => candidates.filter((app) => { const profile = app.user?.employerProfile; return (profile && (profile.name.toLowerCase().includes(searchTerm.toLowerCase()) || app.job.title.toLowerCase().includes(searchTerm.toLowerCase()))); }), [candidates, searchTerm]);
